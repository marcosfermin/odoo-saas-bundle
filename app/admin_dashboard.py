import os, secrets, subprocess, shlex, json, base64, smtplib
...
app.config['SQLALCHEMY_DATABASE_URI'] = f"sqlite:////opt/odoo-admin/admin.db"
...
def has_perm(action: str):
    return current_user.is_authenticated and current_user.role in PERMISSIONS.get(action, set())

def record(action, target="", meta=None):
    entry = AuditLog(actor=(current_user.email if current_user.is_authenticated else "system"),
                     action=action, target=target, meta=json.dumps(meta or {}))
...
def odoo_cli(dbname: str, args: str):
    return run(f"su -s /bin/bash {ODOO_USER} -c \"{shlex.quote(ODOO_VENV)}/bin/python {shlex.quote(ODOO_BIN)} -d {shlex.quote(dbname)} {args}\"")
...
def ensure_lifecycle(bucket: str, prefix: str, days: int):
    ...
    rule = {'ID': rule_id,'Filter': {'Prefix': f"{prefix}/"},'Status': 'Enabled','Expiration': {'Days': days}}
...
def index():
    dbs = get_databases()
    metas = {m.dbname:m for m in TenantMeta.query.filter(TenantMeta.dbname.in_(dbs)).all()}
    rows = []
    for d in dbs:
        ...
        rows.append({'db': d,'url': f"http://{d}.{BASE_DOMAIN}",'size_b': size_b,'users': users,'suspended': suspended,'quota_gb': quota_gb,'over_quota': over_quota})
    status = service_cmd('status')
    return render_template_string(TPL_INDEX, rows=rows, base_domain=BASE_DOMAIN, csrf_token=csrf.dumps('ok'), status=status)
...
def users_create():
    ...
    if User.query.filter_by(email=email).first():
        flash('Email already exists','error'); return redirect(url_for('users_list'))
    pw = bcrypt.hashpw(pwd.encode(), bcrypt.gensalt())
    db.session.add(User(email=email, pw_hash=pw, role=role)); db.session.commit()
    record('user.create', email, {'role': role})
    flash('User created','ok'); return redirect(url_for('users_list'))
...
def users_delete():
    ...
    if uid == current_user.id:
        flash('Cannot delete yourself','error'); return redirect(url_for('users_list'))
    u = User.query.get(uid)
    if not u: abort(404)
    db.session.delete(u); db.session.commit()
    record('user.delete', u.email)
    flash('User deleted','ok'); return redirect(url_for('users_list'))
...
def tenants_create():
    ...
    if not dbname.isalnum():
        flash('Database name must be alphanumeric.','error'); return redirect(url_for('index'))
    ...
    if not TenantMeta.query.filter_by(dbname=dbname).first():
        db.session.add(TenantMeta(dbname=dbname)); db.session.commit()
    record('tenant.create', dbname)
    flash(f"Tenant '{dbname}' created.",'ok'); return redirect(url_for('index'))
...
def tenants_delete():
    ...
    run(f"dropdb {dbname}")
    TenantMeta.query.filter_by(dbname=dbname).delete(); db.session.commit()
    record('tenant.delete', dbname)
    flash(f"Tenant '{dbname}' deleted.",'ok'); return redirect(url_for('index'))
...
def tenants_suspend():
    ...
    if action == 'suspend':
        ...
        m.suspended = True; db.session.add(m); db.session.commit()
        ...
        if m: m.suspended = False; db.session.commit()
...
def tenants_quota():
    ...
    m = TenantMeta.query.filter_by(dbname=dbname).first() or TenantMeta(dbname=dbname)
    m.quota_gb = quota_gb; db.session.add(m); db.session.commit()
    record('tenant.quota.set', dbname, {'quota_gb': quota_gb})
    flash('Quota updated','ok'); return redirect(url_for('index'))
...
def tenants_backup():
    ...
    record('tenant.backup.enqueue', dbname, {'job_id': job.get_id(), 's3': {'bucket': S3_BUCKET, 'key': key}})
...
def modules():
    csrf.loads(request.form.get('_csrf',''))
    dbname = request.form['dbname']
    install = [m.strip() for m in request.form.get('install','').split(',') if m.strip()]
    upgrade = [m.strip() for m in request.form.get('upgrade','').split(',') if m.strip()]
    job = q.enqueue(job_modules, dbname, install, upgrade)
    record('modules.enqueue', dbname, {'install': install, 'upgrade': upgrade, 'job_id': job.get_id()})
...
def billing_webhook():
    ...
    if STRIPE_SIGNING_SECRET and 'Stripe-Signature' in request.headers:
        ...
        p_sig = base64.b64decode(form.pop('p_signature')[0]) if 'p_signature' in form else b''
        ordered = {}
    ...
    dbname = (ev.get('tenant') if isinstance(ev, dict) else ev['data']['object'].get('metadata', {}).get('tenant')) if ev else ''
    ...
    if etype in {'subscription.paused','payment.failed','subscription.canceled'}:
        ...
        run(f"psql -tAc \"ALTER DATABASE {dbname} WITH ALLOW_CONNECTIONS = true;\"")
        m = TenantMeta.query.filter_by(dbname=dbname).first() or TenantMeta(dbname=dbname)
        m.suspended = False; db.session.add(m); db.session.commit()
